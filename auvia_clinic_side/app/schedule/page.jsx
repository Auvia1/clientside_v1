...
/** Maps all known statuses from both versions to badge variants */
function statusVariant(status) {
  const map = {
    booked:      "info",
    confirmed:   "info",
...
/** Human-readable status labels covering both versions */
function statusLabel(status) {
  const map = {
    booked:      "Confirmed",
    confirmed:   "Confirmed",
...
// ─── useLiveActivity ──────────────────────────────────────────────────────────
// Production-ready: uses window.location.host (no hardcoded localhost),
// exponential backoff, mountedRef guard, clinic_id param, full cleanup.
...
function useLiveActivity() {
  ...
  useEffect(() => {
    ...
    // Initial load from REST — includes clinic_id for proper scoping
    fetch(`/api/activity?limit=10&clinic_id=${CLINIC_ID}`)
      .then((r) => r.json())
      .then((json) => { if (json.success && mountedRef.current) setActivities(json.data); })
      .catch(() => {});
    ...
    function connectWs() {
      ...
      // Connects through the Next.js server (which proxies to the private backend)
      // Works in both dev and production — no hardcoded ports
      const proto = window.location.protocol === "https:" ? "wss" : "ws";
      const ws    = new WebSocket(`${proto}://${window.location.host}/ws/activity`);
      wsRef.current = ws;
    ...
    // Small delay before first connect to avoid React StrictMode double-fire issues
    const initTimer = setTimeout(connectWs, 150);
...
// ─── AppointmentCell ──────────────────────────────────────────────────────────
// Actionable for: booked (doc2 status) + confirmed + pending (doc3 statuses)
...
function AppointmentCell({ appt, onStatusChange }) {
  ...
  // Handle status values from both versions of the backend
  const isActionable =
    appt.status === "booked" ||
...
export default function SchedulePage() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f8fb] text-slate-900">
      {/* auto sidebar width — adapts to collapsed/expanded state */}
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[auto_1fr]">
        <Sidebar />
        <main className="flex flex-col gap-6 px-8 py-6">
          <Navbar activeMonitoring={activeMonitoring} onToggleMonitoring={setActiveMonitoring} />
