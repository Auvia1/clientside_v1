...
export default function BuyCreditDialog({ isOpen, onClose, packageData }) {
  ...
  // Pricing calculations (used for display before the order is created)
  const isCustom = packageData?.id === "custom";

  const subtotal = isCustom
    ? parseInt(customAmount || 0, 10) * 5
    : packageData?.price_inr || 0;
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      const creditsToBuy = isCustom ? parseInt(customAmount, 10) : packageData.credits;
      if (isCustom && (isNaN(creditsToBuy) || creditsToBuy <= 0)) {
        setError("Please enter a valid number of credits");
        setLoading(false);
        return;
      }

      const order = await creditsApi.createOrder(clinicId, packageData.id, isCustom ? creditsToBuy : undefined);
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            const paidTotal = order.total_amount?.toFixed
              ? order.total_amount.toFixed(2)
              : (order.amount / 100).toFixed(2);

            setSuccessMessage(
              `${order.credits} credits have been added. Amount Paid: ₹${paidTotal} (including GST)`
            );
            setStep("success");
            refetchBalance();
  ...
  // Prefer backend-provided values once the order has been created, since
  // those are authoritative; fall back to the locally computed estimate
  // before that point.
  const displaySubtotal = orderDetails?.base_amount ?? subtotal;
  const displayGst = orderDetails?.gst_amount ?? gst;
  const displayTotal = orderDetails?.total_amount ?? total;
                ...
                {isCustom ? (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-slate-900">₹{displayGst.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between border-t border-slate-200 pt-3 font-semibold">
                      ...
                      <span className="text-slate-900">₹{displayTotal.toFixed(2)}</span>
                    </div>
                  </>
                ) : (
                  <>
